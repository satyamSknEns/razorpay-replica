...
//         // agar manager h to employees bhi fetch karo
...
//           {/* 👇 Employees list under Teams */}
//           {item.label === "Teams" && employees.length > 0 && (
//             <div className="ml-10 mt-1 text-gray-300 text-sm">
//               {employees.map((emp) => (
//                 <div
//                   key={emp.user.id}
...
const Sidebar = ({
  ...
  const [employees, setEmployees] = useState<Employee[]>([]);
  const [token, setToken] = useState<string | null>(null);
  const [teamsOpen, setTeamsOpen] = useState(false); // 👈 dropdown state
  const cookies = useCookies();
